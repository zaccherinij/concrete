...
use crate::aes::encryptors::{AesBatchedEncryptor, AesCtr, AesKey};
use std::arch::x86_64::{
...
impl AesBatchedEncryptor for Encryptor {
    fn try_new(key: AesKey) -> Result<Self, crate::Error> {
        if is_x86_feature_detected!("aes") && is_x86_feature_detected!("sse2") {
            unsafe {
                ...
                Ok(Encryptor { round_keys })
            }
        } else {
            Err(crate::Error::UnsupportedCpuFeatures {
                features: "aes;sse2",
            })
        }
    }

    fn encrypt_batch(&mut self, AesCtr(aes_ctr): AesCtr) -> [u8; 128] {
...
/// Generates the rounds AES round-keys
///
/// # Safety
///
/// You __must__ make sure the CPU has the `aes` feature
...
/// # Safety
///
/// You must make sure the CPU supports `aes` and `sse2` features
unsafe fn aes_128_key_expansion(key: __m128i, keys: &mut [__m128i; 11]) {
...
mod test {
    use super::*;
    use crate::aes_encryptors::test::*;
    use crate::seeders::hardware::x86_64::RDSeed;
    ...
    #[test]
    fn test_uniformity() {
        crate::aes_encryptors::test::test_uniformity::<Encryptor>(AesKey(
            RDSeed::try_new().unwrap().seed(),
        ));
    }
}
