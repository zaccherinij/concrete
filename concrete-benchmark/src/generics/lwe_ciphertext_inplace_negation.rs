...
use concrete_core::specification::engines::{
    LweCiphertextInplaceNegationEngine, LweCiphertextZeroEncryptionEngine,
    LweSecretKeyGenerationEngine,
};
use concrete_core::specification::entities::{LweCiphertextEntity, LweSecretKeyEntity};
use criterion::{black_box, BenchmarkId, Criterion};

/// A generic function benchmarking the inplace lwe negation operation.
///
/// # Note:
///
...
pub fn bench<
    Engine,           // The benchmarked engine implementing the lwe negation trait.
    InputCiphertext,  // The lwe ciphertext type.
    OutputCiphertext, // The output lwe ciphertext type.
    UtilEngine,       // The util engine used to allocate ciphertexts.
    UtilSecretKey,    // The util secret key used to allocate the ciphertexts.
>(
    c: &mut Criterion,
) where
    Engine: LweCiphertextInplaceNegationEngine<InputCiphertext, OutputCiphertext>,
    ...
    >,
{
    let mut group = c.benchmark_group(benchmark_name!(impl LweCiphertextInplaceNegationEngine<
            InputCiphertext, 
            OutputCiphertext
            > for Engine));
    ...
    for param in PARAMETERS {
        group.bench_with_input(
            ...
            |b, param| {
                ...
                b.iter(|| {
                    unsafe {
                        engine.inplace_neg_lwe_ciphertext_unchecked(
