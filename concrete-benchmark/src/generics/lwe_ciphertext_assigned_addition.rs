...
use concrete_core::specification::engines::{
    LweCiphertextAssignedAdditionEngine, LweCiphertextZeroEncryptionEngine,
    LweSecretKeyGenerationEngine,
};
use concrete_core::specification::entities::{LweCiphertextEntity, LweSecretKeyEntity};
use criterion::{black_box, BenchmarkId, Criterion};

/// A generic function benchmarking the assigned lwe addition operation.
///
/// # Note:
///
...
pub fn bench<
    Engine,           // The benchmarked engine, implementing the allocation trait.
    InputCiphertext,  // The input lwe ciphertext type
    OutputCiphertext, // The output lwe ciphertext type.
    UtilEngine,       // The util engine used to generate the ciphertexts
    UtilSecretKey,    // The secret key used to generate the ciphertexts
>(
    c: &mut Criterion,
) where
    Engine: LweCiphertextAssignedAdditionEngine<InputCiphertext, OutputCiphertext>,
    ...
    >,
{
    let mut group = c.benchmark_group(benchmark_name!(impl LweCiphertextAssignedAdditionEngine<
            InputCiphertext, 
            OutputCiphertext
            > for Engine));
    ...
    for param in PARAMETERS {
        group.bench_with_input(
            ...
            |b, param| {
                ...
                let key = util_engine.generate_lwe_secret_key(lwe_dim).unwrap();
                let mut output = util_engine
                    .zero_encrypt_lwe_ciphertext(&key, VARIANCE)
                    .unwrap();
                let input = util_engine
                    .zero_encrypt_lwe_ciphertext(&key, VARIANCE)
                    .unwrap();

                b.iter(|| {
                    unsafe {
                        engine.assign_add_lwe_ciphertext_unchecked(
