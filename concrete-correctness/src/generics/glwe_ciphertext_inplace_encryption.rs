...
/// A generic function testing the inplace glwe encryption operation.
///
/// # Note:
///
...
pub fn test<
    Engine,          // The tested engine, implementing the inplace encryption trait
    ...
    for (glwe_dimension, polynomial_size, ciphertext_count) in PARAMETERS {
        ...
        for ith in (0..ciphertext_count.0) {
            ...
            let plaintext_vector = util_engine
                ...
                .unwrap();
            engine
                .inplace_encrypt_glwe_ciphertext(
                    &secret_key,
                    &mut encrypted,
                    &plaintext_vector,
                    VARIANCE,
