...
/// A generic function testing the inplace glwe decryption operation.
///
/// # Note:
///
    ...
    UtilRaw: RawUnsignedIntegers,
{
    ...
    for (glwe_dimension, polynomial_size, ciphertext_count) in PARAMETERS {
        ...
        let mut decrypted_plaintext_vector = util_engine
            .create_plaintext_vector(UtilRaw::one_vec(polynomial_size.0).as_slice())
            .unwrap();
        for ith in (0..ciphertext_count.0) {
            ...
            let encrypted = util_engine
                ...
                .unwrap();
            engine
                .inplace_decrypt_glwe_ciphertext(
                    &secret_key,
